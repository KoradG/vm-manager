...
from hypervisor import Hypervisor  # Import Hypervisor class from hypervisor.py
...
class VirtualMachineManagerApp:
    ...
    def create_new_vm(self):
        ...
        disk_size_gb = simpledialog.askinteger("Input", "Enter disk size in GB:")
        if not disk_size_gb or disk_size_gb <= 0:
            messagebox.showerror("Error", "Invalid disk size")
            return

        try:
            self.hypervisor.create_vm(vm_name, iso_path, disk_size_gb)
